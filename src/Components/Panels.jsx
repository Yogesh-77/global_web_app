...
export const SensorCard = ({ s }) => (
  <motion.div layout className='panel p-4 rounded-2xl'>
    ...
    <div className='text-4xl font-bold mt-2' style={{ color: s.color }}>
      {s.value.toFixed(2)}<span className='text-lg ml-1 text-slate-300'>{s.unit}</span>
    </div>
    ...
    <div className='h-16 mt-3'>
      <ResponsiveContainer>
        <LineChart data={s.trend}>
          <Line dataKey='v' stroke={s.color} dot={false} strokeWidth={2} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  </motion.div>
        ...
        <div key={a.id} className={`rounded-xl border p-4 ${levelClass[a.priority]}`}>
          <div className='grid md:grid-cols-[1.4fr_.6fr_.8fr_.7fr] gap-2 items-center'>
            <div>
              ...
              <p className='text-slate-400'>{a.zone} • {a.ts}</p>
            </div>
            <div className={`text-center border rounded px-2 py-1 font-bold ${pillClass[a.priority]}`}>
              {a.priority.toUpperCase()}
            </div>
            <div className='text-cyan-200'>→ {a.action}</div>
            <select className='select' value={a.priority} onChange={(e) => onPriorityChange(a.id, e.target.value)}>
      ...
      <div>
        ...
        <div className='mt-2 flex gap-2 items-center'>
          <select className='select' value={emotion} onChange={(e) => setEmotion(e.target.value)}>
            {['Normal', 'Focused', 'Stressed', 'Fatigued', 'Overloaded'].map((x) => <option key={x}>{x}</option>)}
          </select>
          <span className='text-sm text-cyan-200'>Confidence: {emotionConfidence}%</span>
        </div>
      </div>
      <div>
        <div className='flex gap-2'>
          <button className='btn' onClick={onStartVoice}><Mic size={14} /> Start</button>
          <button className='btn' onClick={onStopVoice}><MicOff size={14} /> Stop</button>
          <span className={`${listening ? 'text-green-400' : 'text-slate-400'} text-sm`}>{listening ? 'Listening' : 'Idle'}</span>
        </div>
        <p className='mt-3 text-sm border border-cyan-500/20 rounded p-2 min-h-12'>
          {transcript || 'Say: show critical alarms, activate emergency mode, simplify dashboard, reset alarms, system status, start, stop'}
        </p>
        <p className='text-sm mt-2 text-indigo-300'>AI: {aiResponse}</p>
        <div className='text-xs mt-2 space-y-1 max-h-20 overflow-auto'>
...
export const RightCharts = ({ alarmFreq, categories, cognitiveTrend, cognizantScore }) => (
  <section className='space-y-4'>
    <div className='panel p-4 rounded-2xl'>
      <h3 className='title'>ALARM FREQUENCY MATRIX</h3>
      <div className='h-44'>
        <ResponsiveContainer>
          <BarChart data={alarmFreq}>
            <XAxis dataKey='hour' tick={{ fill: '#94a3b8', fontSize: 11 }} />
            <Tooltip contentStyle={{ background: '#08142c', border: '1px solid #1d4ed8' }} />
    ...
    <div className='panel p-4 rounded-2xl'>
      <h3 className='title'>COGNIZANT RESPONSE FREQUENCY</h3>
      <div className='h-36'>
        <ResponsiveContainer>
          <AreaChart data={cognitiveTrend}>
            <XAxis dataKey='t' tick={{ fill: '#94a3b8', fontSize: 10 }} />
            <Tooltip contentStyle={{ background: '#08142c', border: '1px solid #7c3aed' }} />
            <Area dataKey='focus' stroke='#22c55e' fill='#22c55e33' />
            <Area dataKey='fatigue' stroke='#a855f7' fill='#a855f733' />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      <div className='mt-2 text-sm text-cyan-200'>
        Cognizant Stability Index: <span className='text-emerald-300 font-semibold'>{cognizantScore}%</span>
      </div>
    </div>

    <div className='panel p-4 rounded-2xl'>
      <h3 className='title'>ALARM CATEGORIES</h3>
      <div className='h-44'>
        <ResponsiveContainer>
          <PieChart>
            <Pie data={categories} dataKey='value' innerRadius={35} outerRadius={70}>
