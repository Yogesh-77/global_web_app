...
import { LineChart, Line, ResponsiveContainer, BarChart, Bar, PieChart, Pie, Cell } from 'recharts'
...
export const RightCharts = ({ alarmFreq, categories }) => <section className='space-y-4'><div className='panel p-4 rounded-2xl'><h3 className='title'>ALARM FREQUENCY</h3><div className='h-40'><ResponsiveContainer><BarChart data={alarmFreq}><Bar dataKey='Critical' fill='#ef4444' /><Bar dataKey='High' fill='#f97316' /><Bar dataKey='Medium' fill='#eab308' /><Bar dataKey='Low' fill='#22c55e' /></BarChart></ResponsiveContainer></div></div><div className='panel p-4 rounded-2xl'><h3 className='title'>ALARM CATEGORIES</h3><div className='h-44'><ResponsiveContainer><PieChart><Pie data={categories} dataKey='value' innerRadius={35} outerRadius={70}>{categories.map((c) => <Cell key={c.name} fill={c.color} />)}</Pie></PieChart></ResponsiveContainer></div></div></section>
